...
const clickListener = function (ev) {
    ...
    if (el && el === ev.target) {
		...
		label.innerHTML = '<div class="animated-button1" style="background-image:url('+sfondo+')">'+name+'<br>'+testo+'<audio controls><source src="'+link+'" type="audio/mp3"></audio><button onclick="myFunction()">Remove</button></div><span></span><span></span><span></span><span></span></div>';
        ...
        function myFunction() {
  const element = document.getElementById("place-label");
  element.remove();
}